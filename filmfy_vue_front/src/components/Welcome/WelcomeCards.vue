<template>
  <div class="container-fluid">
    <!--Card1-->
    <div class="card mb-3" >
      <div class="row g-0">
        <div class="col-md-4">
          <img
            src="/filmfy_vue_front/src/assets/img/Welcome/Share all Lists.png"
            class="img-fluid rounded-start"
            alt="..."
          />
        </div>
        <div class="col-md-8">
          <div class="card-body">
            <h5 class="card-title">
              Crea Listas <br />
              y compártelas
            </h5>
            <p class="card-text">
              Se el creador de tu propio repertorio <br />
              y comparte tus listas con tus amigos.
            </p>
          </div>
        </div>
      </div>
    </div>
    <!--Card2-->
    <div class="card mb-3">
      <div class="row g-0">
        <div class="col-md-8">
          <div class="card-body">
            <h5 class="card-title">
              Se un referente <br />
              de Cine
            </h5>
            <p class="card-text">
              ¡Sé el referente de tu contenido favorito! <br />
              Estate en todo debate de este arte
            </p>
          </div>
        </div>
        <div class="col-md-4">
          <img
            src="/filmfy_vue_front/src/assets/img/Welcome/ReferenteDeCine.png"
            class="img-fluid rounded-start"
            alt="..."
          />
        </div>
      </div>
    </div>
    <!--Card3-->
    <div class="card mb-3">
      <div class="row g-0">
        <div class="col-md-4">
          <img
            src="/filmfy_vue_front/src/assets/img/Welcome/Content News.png"
            class="img-fluid rounded-start"
            alt="..."
          />
        </div>
        <div class="col-md-8">
          <div class="card-body">
            <h5 class="card-title">
              Enterate de las ultimas <br />
              novedades
            </h5>
            <p class="card-text">
              No te pierdas ni un solo detalle y <br />
              novedad de ninuno de tus artistas y contenidos favoritos.
            </p>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "WelcomeCards",
};
</script>

<style>
</style>